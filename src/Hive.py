...
class Hive():
    ...
    def __init__(self, Eyeriss, mode="auto"):
        self.mode = mode
    ...
    def Conv2d(self, Pictures=0, FilterWeights=0):
        ...
        Psum = [self.Eyeriss.Conv2d(ps, ofmapWidth, self.n, self.p, self.q) for ps in Passes]
        self.Reverse(Psum)
        OfMaps = self.Output()
    ...
    def __PEArrayMapping__(self):
        PESetHeight = self.FilterWeights.shape[2]
        PESetWidth = self.Pictures.shape[2]- self.FilterWeights.shape[2] + 1
